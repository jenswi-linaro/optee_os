		     ...
		     struct mobj_spci *ms)
{
	...
	for (n = 0; n < num_regions; n++) {
		...
		if (mobj_spci_add_pages_at(ms, &idx, addr, page_count))
			return SPCI_INVALID_PARAMETER;
	}
...
static int add_mem_share(struct spmc_mem_region_descr *descr,
			 size_t descr_len, uint32_t *global_handle)
{
	...
	ms = mobj_spci_sel1_spmc_new(num_pages);
	if (!ms)
	...
	*global_handle = mobj_spci_push_to_inactive(ms);
...
static void handle_mem_share(struct thread_smc_args *args)
{
	...
	ret_val = add_mem_share((void *)(tee_mm_get_smem(mm) + offs),
				len, &global_handle);
	if (!ret_val) {
...
static void handle_mem_reclaim(struct thread_smc_args *args)
{
	...
	switch (mobj_spci_sel1_spmc_reclaim(args->a1)) {
	case TEE_SUCCESS:
		ret_fid =  SPCI_SUCCESS;
		ret_val = 0;
		break;
	case TEE_ERROR_ITEM_NOT_FOUND:
		DMSG("cookie %#"PRIx32" not found", (uint32_t)args->a1);
		ret_val = SPCI_INVALID_PARAMETER;
		break;
	default:
		DMSG("cookie %#"PRIx32" busy", (uint32_t)args->a1);
		ret_val = SPCI_DENIED;
		break;
	}
out:
...
{
	uint32_t res = mobj_spci_unregister_by_cookie(cookie);
...
static void thread_rpc_free(unsigned int bt, uint32_t cookie, struct mobj *mobj)
{
	...
	mobj_put(mobj);
	res = mobj_spci_unregister_by_cookie(cookie);
	if (res)
		DMSG("mobj_spci_unregister_by_cookie(%#"PRIx32"): res %#"PRIx32,
		     cookie, res);
	thread_rpc(&rpc_arg);
}
